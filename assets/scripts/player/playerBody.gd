...
onready var player_script = get_node("../")
...
func playerMakingShy():
	if get_node("Armature/Skeleton/coat").visible:
		if legsHit.hitting && !legsHit.temp_front:
...
#			walk_offset += 0.06
		...
		if player_script.crouching && crouching_cooldown > 0:
			if walk_offset < 0.3:
				walk_offset += 0.02
			elif walk_offset > 0.4:
				walk_offset -= 0.03
	...
	if player_script.stats.Health > 0:
		...
		if body_rot > 130 || body_rot < -105:
			if smile_cooldown > 0:
				...
				smile_cooldown = 5
				player_script.stats.head.smileOn()
		else:
			if smile_cooldown != 5:
				smile_cooldown = 5
				player_script.stats.head.smileOff()
		
		if player_script.have_coat && playerMakingShy():
			if shy_cooldown > 0:
				shy_cooldown -= delta
			else:
				shy_cooldown = 1.5
				player_script.stats.head.shyOn()
		...
		#--rotating body--
		if G.player.mayMove:
			if Input.is_action_pressed("ui_left") && (!player_script.flying || player_script.flying_fast):
				body_rot = 90.0
				if Input.is_action_pressed("ui_up"):
					body_rot = 45.0
				elif Input.is_action_pressed("ui_down"):
					body_rot = -45.0
			if Input.is_action_pressed("ui_right") && (!player_script.flying || player_script.flying_fast):
				body_rot = -90.0
				if Input.is_action_pressed("ui_up"):
					body_rot = -45.0
				elif Input.is_action_pressed("ui_down"):
...
func _input(event):
	if event is InputEventMouseMotion && Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		var mouse_sens = player_script.MOUSE_SENSITIVITY
