...
#диалоговые переменные для неписей
...
func loadDialogueLang():
	var lang = "ru/"
	if G.english:
		lang = "en/"
	dialogue_path = "res://assets/dialogues/json/" + lang + dialogue_path


func changeDialogueStage(newStage):
	if has_node("dialogueScript"):
		get_node("dialogueScript").changeDialogueStage(newStage)
